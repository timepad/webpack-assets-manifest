...
class WebpackAssetsManifest
{
  ...
  /**
   * `JSON.stringify()` or `Yaml.dump()` the manifest.
   *
   * @return {string}
   */
  toString()
  {
    switch (this.options.serializeTo) {
      case 'json':
        return JSON.stringify(this, this.options.replacer, this.options.space) || '{}';
      case 'yaml':
        return yaml.dump(this.toJSON(), {replacer: this.options.replacer, indent: this.options.space}) || '{}';
    }
  }
  ...
  maybeMerge()
  {
    if ( this.options.merge ) {
      try {
        ...
        const data = this._readFile(this.getOutputPath());
